defmodule MechanicsTest do
  ...
  test "select action when passing a space with multiple actions" do
    ...
    assert {select1_state.state, select1_state} == Bathysphere.Game.Mechanics.down(game_state, 6)
    assert {select2_state.state, select2_state} == Bathysphere.Game.Mechanics.select_action(select1_state, {{:stress, -1, false}, 1})
    assert {final_state.state, final_state} == Bathysphere.Game.Mechanics.select_action(select2_state, {{:oxygen, -1, false}, 1})
  end
